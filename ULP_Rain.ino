...
static uint32_t getPulseCount(void) {
  /* Le programme ULP compte les fronts du signal.
   * Un basculement du pluviomètre génère deux fronts
   * (montant + descendant), on divise donc par 2
   * pour obtenir le nombre réel d’impulsions.
   */
  uint32_t pulse_count_from_ulp = (ulp_edge_count & UINT16_MAX) / 2;
  
  /* on conserve le front restant pour le prochain cycle afin d’éviter une perte de comptage.*/
...
static uint32_t getShortestPulse(void) {
  /* Le programme ULP enregistre la durée minimale d’une impulsion. */
  uint32_t pulse_time_min = (ulp_pulse_min & UINT16_MAX) * ULPSLEEP;
  
  /*  Réinitialisation de la valeur minimale après lecture*/
  ulp_pulse_min = 0;
...
long getElapsed(unsigned long compareMillis) {
  ...
  return labs((long)elapsedMillis);
